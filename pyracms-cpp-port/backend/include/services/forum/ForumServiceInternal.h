...
// $1 = acting user id: author of the row, or someone who moderates the
// site the row lives in (see canModerateSql).
const std::string kThreadOwnerOrMod =
    "(user_id = $1::int OR " +
    canModerateSql("$1", threadTenantSql("forum_threads")) + ")";
const std::string kPostOwnerOrMod =
    "(user_id = $1::int OR " +
    canModerateSql("$1", postTenantSql("forum_posts")) + ")";
} // namespace
