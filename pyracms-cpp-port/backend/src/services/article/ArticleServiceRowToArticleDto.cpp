...
ArticleDto ArticleService::rowToArticleDto(const drogon::orm::Row &row) {
    ...
    dto.rendererName = row["renderer_name"].isNull()
                           ? "markdown"
                           : row["renderer_name"].as<std::string>();
    dto.viewCount = row["view_count"].as<int>();
    dto.createdAt = row["created_at"].as<std::string>();
    dto.status =
        row["status"].isNull() ? "published" : row["status"].as<std::string>();
    dto.publishedAt = row["published_at"].isNull()
                          ? ""
                          : row["published_at"].as<std::string>();
    dto.scheduledAt = row["scheduled_at"].isNull()
                          ? ""
                          : row["scheduled_at"].as<std::string>();
    return dto;
}
