    ...
    db->execSqlAsync(
        ...
        [this, db, articleId, tags, cb](const drogon::orm::Result &) {
            ...
            for (size_t i = 0; i < tags.size(); ++i) {
                placeholders.push_back("($" + std::to_string(paramIdx++) +
            ...
            auto remaining =
                std::make_shared<int>(static_cast<int>(tags.size()));
            auto failed = std::make_shared<bool>(false);
            auto errorMsg = std::make_shared<std::string>();

            for (const auto &tag : tags) {
                db->execSqlAsync(
                    "INSERT INTO article_tags (article_id, name) VALUES ($1, "
                    "$2)",
