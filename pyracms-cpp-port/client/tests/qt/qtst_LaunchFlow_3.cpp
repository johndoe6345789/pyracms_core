...
void TstLaunchFlow::missingExecutableIsReported()
{
    QVERIFY(buildZip(m_dir.filePath("n.zip"), {{"readme.txt", "x"}}));
    m_http.routes["/n.zip"] = readAll(m_dir.filePath("n.zip"));
    install("noexe", "/n.zip", true, "missing.bin");
    QSignalSpy err(m_games, &GameManager::gameError);
    m_games->launchGame("noexe");
    QCOMPARE(err.count(), 1);
    QVERIFY(err.at(0).at(1).toString().contains("missing.bin"));
}
