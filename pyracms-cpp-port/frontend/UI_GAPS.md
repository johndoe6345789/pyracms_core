...
| Admin Backup page (follow-up) | Export now reads real tenant settings (`GET /api/settings?tenant_id=`) and menu groups/items; settings import applies via `PUT /api/settings/{name}?tenant_id=` | **fixed** - menu import shows "not available yet" (restoring groups/items would duplicate rows; no bulk/replace route on `/api/menu-groups`) |
| Admin Styles / Templates Save | Save `console.log`ged and persisted nothing | **made honest** - Save disabled with "not available yet" notice; needs a backend theme/template route (or a settings key the tenant layout consumes) |
| Admin dashboard stats | Counted the first tenant's data and showed `0` on errors | **fixed** - uses the current tenant, shows `N/A` on failure; dropped the platform-level "Tenants" card |
| Admin recent activity / activity chart / system health | Hard-coded fake data in unused `AdminDashboard` tree, plus the unused `DragDropMenuEditor`/`placeholderMenu` tree | deferred - unreachable from any page; safe to delete (`components/admin/{AdminDashboard,ActivityChart,RecentActivityList,SystemHealthPanel,QuickActionsPanel,StatCard,DragDropMenuEditor,useMenuHandlers,placeholderMenu,...}`); a real feed needs an audit-log/activity route |
| Gallery picture set-cover / delete | Failures were swallowed silently | **fixed** - error Alert via `apiErrorMessage` |
| Hypernucleus launcher not linked from the site | Nothing pointed visitors at the desktop client that the `pyracms://` Play/Install links need | **fixed** - `/download` and `/site/[slug]/download` (latest GitHub release, OS/arch detection, all-platform table with size + SHA-256 from the release `digest`, unsigned-binary install notes, deep-link explanation, Releases fallback); "Get the launcher" in the top bar and burger drawer, banner on Games, link next to Play/Install, portal hero button, new `SiteFooter` |
| Hypernucleus SHA256SUMS in the table | Checksums come from GitHub's asset `digest`; `SHA256SUMS` is only linked, not fetched (release downloads redirect to hosts outside the CSP `connect-src`) | deferred - add the download hosts to the CSP if fetching the file is wanted |
| "Fork me on GitHub" ribbon | None | **fixed** - accessible `ForkRibbon` (corner on portal hero, download page and site pages; inline in the footer), repo from `NEXT_PUBLIC_GITHUB_REPO` |
