...
test.describe('Authenticated create-site form', () => {
  ...
  test('create-site form fields are all visible', async ({ page }) => {
    await expect(page.getByTestId('create-site-form')).toBeVisible()
    await expect(page.getByTestId('site-name-input')).toBeVisible()
    await expect(page.getByTestId('site-slug-input')).toBeVisible()
    await expect(page.getByTestId('site-description-input')).toBeVisible()
    await expect(page.getByTestId('create-site-submit')).toBeVisible()
  })

  test('site-name input accepts typed text', async ({ page }) => {
  ...
  test('site-slug input accepts typed text', async ({ page }) => {
    const input = page.getByTestId('site-slug-input')
    await input.fill('my-awesome-site')
    await expect(input).toHaveValue('my-awesome-site')
  })

  test('site-description input accepts multi-line text', async ({ page }) => {
    const input = page.getByTestId('site-description-input')
    await input.fill('Line one\nLine two')
    await expect(input).toHaveValue('Line one\nLine two')
  })

  test(
    'slug helper text shows "/site/<slug>" preview ' + 'when slug has a value',
    async ({ page }) => {
