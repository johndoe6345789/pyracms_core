...
test.describe('Authenticated create-site form', () => {
  ...
  test('invalid slug pattern shows an error', async ({ page }) => {
    const slugInput = page.getByTestId('site-slug-input')

    // Clear auto-filled slug first
    await page.getByTestId('site-name-input').fill('Test')
  ...
  test(
    'slug with uppercase characters is blocked or ' + 'normalised',
    async ({ page }) => {
      const slugInput = page.getByTestId('site-slug-input')
