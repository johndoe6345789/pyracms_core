...
test.describe('Authenticated create-site form', () => {
  ...
  test('mocked success redirects to /site/{slug}', async ({ page }) => {
    ...
    await page.getByTestId('site-name-input').fill('Mock Site')
    await page.getByTestId('site-name-input').press('Tab')

    await page.getByTestId('create-site-submit').click()

    await page.waitForURL('/site/**', { timeout: 8_000 }).catch(() => {
