...
test.describe('Guest flow — unauthenticated user', () => {
  ...
  test('Visiting /create-site directly shows AuthPromptCard', async ({
    page,
  }) => {
    ...
    await expect(signIn).toHaveAttribute('href', '/auth/login/create-site')

    // "Register" button must also be present
    const register = page.getByTestId('prompt-register-button')
    await expect(register).toBeVisible()
    await expect(register).toHaveAttribute('href', '/auth/register/create-site')
  })

  test(
    'Sign In button on AuthPromptCard navigates to ' + 'the login page',
    async ({ page }) => {
      await page.goto('/create-site')
  ...
  test(
    'Register button on AuthPromptCard navigates to ' + 'the register page',
    async ({ page }) => {
      await page.goto('/create-site')
      await page.getByTestId('prompt-register-button').click()
