...
test.describe('Login page — /auth/login/create-site', () => {
  test('forgot-password link is keyboard-accessible', async ({ page }) => {
  ...
  test(
    'submit button text is "Login" or "Sign In" ' + '(visible label present)',
    async ({ page }) => {
      await page.goto('/auth/login/create-site')
  ...
  test('successful login redirects to /create-site', async ({ page }) => {
  ...
  test('invalid credentials show an error message', async ({ page }) => {
    await page.goto('/auth/login/create-site')
    await page.getByTestId('username-input').fill('nobody')
  ...
  test('password field accepts typed text (masked)', async ({ page }) => {
    await page.goto('/auth/login/create-site')
