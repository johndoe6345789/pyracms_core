...
test.describe('Register flow — /auth/register/create-site', () => {
  test('all register inputs accept typed text', async ({ page }) => {
    await page.goto('/auth/register/create-site')

    await page.getByTestId('register-username-input').fill('jane')
    await expect(page.getByTestId('register-username-input')).toHaveValue(
      'jane',
    )
    ...
    await page.getByTestId('register-password-input').fill('Secret1!')
    await expect(page.getByTestId('register-password-input')).toHaveValue(
      'Secret1!',
    )

    await page.getByTestId('register-confirm-password-input').fill('Secret1!')
    await expect(
      page.getByTestId('register-confirm-password-input'),
    ).toHaveValue('Secret1!')

    await page.getByTestId('register-firstname-input').fill('Jane')
    await expect(page.getByTestId('register-firstname-input')).toHaveValue(
      'Jane',
    )
  ...
  test('password-strength bar appears after typing', async ({ page }) => {
    await page.goto('/auth/register/create-site')
