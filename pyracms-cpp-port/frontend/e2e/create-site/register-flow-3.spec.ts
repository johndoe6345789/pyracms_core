...
test.describe('Register flow — /auth/register/create-site', () => {
  test('password "abc" (3 chars) shows "Weak" strength', async ({ page }) => {
  ...
  test('password "abcde123" shows "Fair" strength', async ({ page }) => {
    await page.goto('/auth/register/create-site')
    await page.getByTestId('register-password-input').fill('abcde123')
    // score=3: length≥8 + digit + lowercase
    await expect(page.getByTestId('password-strength-label')).toHaveText('Good')
  })

  test('password "abcdeABC123" shows "Strong" strength', async ({ page }) => {
    await page.goto('/auth/register/create-site')
    await page.getByTestId('register-password-input').fill('abcdeABC123')
  ...
  test(
    ...
    async ({ page }) => {
      await page.goto('/auth/register/create-site')
      await page.getByTestId('register-password-input').fill('test')
  ...
  test(
    'password-strength bar is hidden when password ' + 'field is empty',
    async ({ page }) => {
      await page.goto('/auth/register/create-site')
      // Field starts empty — bar should not be in DOM
      await expect(page.getByTestId('password-strength')).not.toBeAttached()
    },
  )
})
