...
test.describe('Login — /auth/login', () => {
  test(
    ...
    async ({ page }) => {
      ...
      await page.getByTestId('username-input').fill('baduser')
      await page.getByTestId('password-input').fill('badpass')
  ...
  test('username-input has aria-label', async ({ page }) => {
    await page.goto('/auth/login')
  ...
  test('forgot-password-link has aria-label', async ({ page }) => {
    await page.goto('/auth/login')
