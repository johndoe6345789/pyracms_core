...
test.describe('Portal — /', () => {
  ...
  test('portal-page has role="main" and aria-label', async ({ page }) => {
    await page.goto('/')

    const root = page.getByTestId('portal-page')
    await expect(root).toHaveAttribute('role', 'main')
    await expect(root).toHaveAttribute('aria-label', /portal homepage/i)
  })

  test('create-site-button has an accessible aria-label', async ({ page }) => {
    await page.goto('/')
  ...
  test('portal-page "Available Sites" section heading is visible', async ({
    page,
  }) => {
    await page.goto('/')
  ...
  test('create-site-button is keyboard-focusable (tabIndex)', async ({
    page,
  }) => {
    await page.goto('/')
