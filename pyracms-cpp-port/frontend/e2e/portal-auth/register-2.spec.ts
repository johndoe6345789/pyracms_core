...
test.describe('Register — /auth/register', () => {
  test('mismatched passwords show register-error', async ({ page }) => {
  ...
  test(
    'successful registration navigates away from ' + '/auth/register',
    async ({ page }) => {
      ...
      const navigated = await page
        .waitForURL((url) => !url.pathname.startsWith('/auth/register'), {
          timeout: 10_000,
        })
        .then(() => true)
        .catch(() => false)
