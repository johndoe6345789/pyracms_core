...
test.describe('Register — /auth/register', () => {
  test('each register input has an aria-label', async ({ page }) => {
    ...
    for (const tid of testIds) {
      ...
      expect(label, `Expected aria-label on ${tid}`).toBeTruthy()
    }
  })

  test('register-submit has aria-label', async ({ page }) => {
    await page.goto('/auth/register')
  ...
  test('login-link has aria-label', async ({ page }) => {
    await page.goto('/auth/register')
  ...
  test('register-submit is disabled while submitting', async ({ page }) => {
