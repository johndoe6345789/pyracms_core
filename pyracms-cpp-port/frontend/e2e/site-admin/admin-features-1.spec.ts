...
test.describe('Admin Features', () => {
  ...
  test('"Save Changes" button is present', async ({ page }) => {
    await expect(page.getByTestId('save-features-btn')).toBeVisible()
  })

  test('feature card for "articles" is rendered', async ({ page }) => {
    await expect(page.getByTestId('feature-card-articles')).toBeVisible()
  })

  test('feature card for "forum" is rendered', async ({ page }) => {
    await expect(page.getByTestId('feature-card-forum')).toBeVisible()
  })

  test('feature toggle for "articles" is checked (enabled=true)', async ({
    page,
  }) => {
