...
test.describe('Admin Templates', () => {
  ...
  test('toggling Preview button hides live preview pane', async ({ page }) => {
    ...
    await expect(page.getByText(/live preview/i)).not.toBeVisible()
  })
  ...
  test('live preview pane renders default header HTML', async ({ page }) => {
    await expect(page.getByText(/live preview - header/i)).toBeVisible()
  })

  test('Monaco editor container is mounted', async ({ page }) => {
    await page
