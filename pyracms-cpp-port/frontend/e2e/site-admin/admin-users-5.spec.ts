...
test.describe('Admin Users', () => {
  ...
  test('submit button is enabled when required fields are filled', async ({
    ...
    await page.getByTestId('new-username-input').fill('newuser')
    await page.getByTestId('new-email-input').fill('new@example.com')
    await page.getByTestId('new-password-input').fill('secret123')
    await expect(page.getByTestId('submit-create-btn')).toBeEnabled()
  })
  ...
  test('admin sidebar is present on users page', async ({ page }) => {
