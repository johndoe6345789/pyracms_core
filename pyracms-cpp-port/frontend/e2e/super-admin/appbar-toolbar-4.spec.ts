...
test.describe('AppBar toolbar', () => {
  ...
  test('clicking "Sign Out" logs out and redirects to /', async ({ page }) => {
    await page.getByTestId('user-bubble-btn').click()
    await page.getByTestId('logout-btn').click()
    await expect(page).toHaveURL('/')
  })

  test('UserBubble avatar button is keyboard-activatable', async ({ page }) => {
  ...
  test(
    'unauthenticated user sees guest-login-link ' + 'instead of avatar',
    async ({ page: guestPage }) => {
      ...
      const guestChip = guestPage.getByTestId('guest-login-link')
      const isVisible = await guestChip.isVisible().catch(() => false)
      if (isVisible) {
        await expect(guestChip).toHaveAttribute('href', '/auth/login')
      }
    },
  )
})
