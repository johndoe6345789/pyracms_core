...
test.describe('Tenant management', () => {
  ...
  test('filter input clears with keyboard (Ctrl+A Delete)', async ({
    page,
  }) => {
    ...
    await expect(page.getByTestId('tenant-row-beta')).not.toBeVisible()
    ...
    await expect(page.getByTestId('tenant-row-alpha')).toBeVisible()
    await expect(page.getByTestId('tenant-row-beta')).toBeVisible()
  })

  test('filter input has accessible aria-label', async ({ page }) => {
    const filterInput = page.getByTestId('tenant-filter-input')
    // The nested input carries the aria-label
    await expect(filterInput.locator('input')).toHaveAttribute(
      'aria-label',
      'Filter tenants',
    )
  })
