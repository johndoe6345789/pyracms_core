...
test.describe('Article detail — /site/demo/articles/hello-world', () => {
  test('page loads without a fatal JS error', async ({ page }) => {
  ...
  test('shows article content when API returns data', async ({ page }) => {
  ...
  test('article title heading is visible', async ({ page }) => {
    await page.route('**/api/tenants**', (route) =>
      route.fulfill({ json: MOCK_TENANT }),
    )
    await page.route('**/api/articles/hello-world**', (route) =>
  ...
  test('article content wrapper has aria-label', async ({ page }) => {
    await page.route('**/api/tenants**', (route) =>
      route.fulfill({ json: MOCK_TENANT }),
    )
    await page.route('**/api/articles/hello-world**', (route) =>
    ...
    await page.goto(`${BASE}/articles/hello-world`)
    await expect(page.getByTestId('article-content-wrapper')).toBeVisible({
      timeout: 8_000,
    })
  })
})
