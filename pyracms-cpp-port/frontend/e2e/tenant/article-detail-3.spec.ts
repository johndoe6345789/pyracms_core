...
test.describe('Article detail — /site/demo/articles/hello-world', () => {
  test('Dislike button is visible and has aria-label', async ({ page }) => {
  ...
  test('Like count is displayed', async ({ page }) => {
    await page.route('**/api/tenants**', (route) =>
      route.fulfill({ json: MOCK_TENANT }),
    )
    await page.route('**/api/articles/hello-world**', (route) =>
    ...
    await page.goto(`${BASE}/articles/hello-world`)
    await expect(page.getByTestId('like-count')).toHaveText('5', {
      timeout: 8_000,
    })
  })

  test('clicking Like button does not crash', async ({ page }) => {
    await page.route('**/api/tenants**', (route) =>
      route.fulfill({ json: MOCK_TENANT }),
    )
    await page.route('**/api/articles/hello-world**', (route) =>
    ...
    await page.goto(`${BASE}/articles/hello-world`)
    await page.getByTestId('like-btn').click({ timeout: 8_000 })
    cleanup()
