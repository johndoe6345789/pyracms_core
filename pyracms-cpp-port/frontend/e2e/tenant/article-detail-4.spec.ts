...
test.describe('Article detail — /site/demo/articles/hello-world', () => {
  test('article actions section has correct ARIA region', async ({ page }) => {
    ...
    await expect(page.getByTestId('article-actions')).toBeVisible({
      timeout: 8_000,
    })
  })

  test('vote buttons section has correct ARIA container', async ({ page }) => {
    await page.route('**/api/tenants**', (route) =>
      route.fulfill({ json: MOCK_TENANT }),
    )
    await page.route('**/api/articles/hello-world**', (route) =>
    ...
    await page.goto(`${BASE}/articles/hello-world`)
    await expect(page.getByTestId('article-vote-buttons')).toBeVisible({
      timeout: 8_000,
    })
  })

  test('shows "not found" state or null when article missing', async ({
    page,
  }) => {
    await page.route('**/api/tenants**', (route) =>
      route.fulfill({ json: MOCK_TENANT }),
