...
test.describe('Article edit — /site/demo/articles/hello-world/edit', () => {
  ...
  test('"Cancel" button links back to article', async ({ page }) => {
    await page.goto(`${BASE}/articles/hello-world/edit`)
    const cancelBtn = page.getByTestId('cancel-edit-btn')
  ...
  test('title input can receive edited text', async ({ page }) => {
    await page.goto(`${BASE}/articles/hello-world/edit`)
  ...
  test('edit form section has ARIA label', async ({ page }) => {
    await page.goto(`${BASE}/articles/hello-world/edit`)
    await expect(
