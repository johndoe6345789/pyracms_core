...
test.describe('Articles list — /site/demo/articles', () => {
  ...
  test('search bar is present', async ({ page }) => {
    await page.goto(`${BASE}/articles`)
    await expect(page.getByTestId('article-search-input')).toBeVisible()
  })

  test('search bar has correct accessible label', async ({ page }) => {
    await page.goto(`${BASE}/articles`)
    await expect(page.getByLabel('Search articles')).toBeVisible()
  })

  test('"Create Article" button is visible', async ({ page }) => {
    await page.goto(`${BASE}/articles`)
    await expect(page.getByTestId('create-article-btn')).toBeVisible()
  })

  test('"Create Article" button links to the create page', async ({ page }) => {
    await page.goto(`${BASE}/articles`)
    await expect(page.getByTestId('create-article-btn')).toHaveAttribute(
      'href',
