...
test.describe('Forum thread list — /site/demo/forum/1', () => {
  ...
  test('thread pagination is rendered', async ({ page }) => {
    await page.goto(`${BASE}/forum/1`)
    await expect(page.getByTestId('thread-pagination')).toBeVisible()
  })

  test('thread table has accessible aria-label', async ({ page }) => {
    await page.goto(`${BASE}/forum/1`)
    await expect(
  ...
  test('thread row links to thread view page', async ({ page }) => {
    await page.route('**/api/forum/threads**', (route) =>
      route.fulfill({ json: MOCK_THREADS }),
    )
    await page.goto(`${BASE}/forum/1`)
    await expect(page.getByTestId('thread-row-1')).toBeVisible({
      timeout: 8_000,
    })
  })
