...
test.describe('Keyboard navigation & ARIA — cross-page', () => {
  test('snippets page "New Snippet" button is keyboard-activatable', async ({
    page,
  }) => {
    ...
    const newBtn = page.getByTestId('new-snippet-btn')
    await newBtn.focus()
  ...
  test('forum thread create form fields all have labels', async ({ page }) => {
  ...
  test('article create form fields all have accessible labels', async ({
    page,
  }) => {
    await page.route('**/api/tenants**', (route) =>
      route.fulfill({ json: MOCK_TENANT }),
    ...
    await expect(page.getByLabel('Revision Summary')).toBeVisible()
  })
})
