...
test.describe('Snippets — /site/demo/snippets', () => {
  ...
  test('snippet search input accepts text', async ({ page }) => {
    await page.goto(`${BASE}/snippets`)
    const searchInput = page.getByTestId('snippet-search')
    await searchInput.fill('fibonacci')
    await expect(searchInput).toHaveValue('fibonacci')
  })
  ...
  test('empty state message is shown when no snippets exist', async ({
    page,
  }) => {
    await page.goto(`${BASE}/snippets`)
    await expect(page.getByTestId('no-snippets-msg')).toBeVisible()
  })

  test('"Back to Site" button is present', async ({ page }) => {
    await page.goto(`${BASE}/snippets`)
    await expect(page.getByText('Back to Site')).toBeVisible()
  })
})
