...
test.describe('User profile — /site/demo/users/admin', () => {
  test('shows user profile when API returns data', async ({ page }) => {
  ...
  test('shows "User not found" when API returns empty', async ({ page }) => {
  ...
  test('profile tabs are rendered when user exists', async ({ page }) => {
    await page.route('**/api/users**', (route) =>
  ...
  test('Followers tab is rendered', async ({ page }) => {
    await page.route('**/api/users**', (route) =>
      route.fulfill({ json: [MOCK_USER] }),
    )
    await page.goto(`${BASE}/users/admin`)
