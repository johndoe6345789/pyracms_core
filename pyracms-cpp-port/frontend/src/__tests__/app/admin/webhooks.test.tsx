...
const hook = { id: 7, url: 'https://x.io/h', active: true,
  events: ['article.created'], createdAt: '2026-01-01T00:00:00Z' }

beforeEach(() => {
  jest.resetAllMocks()
  m.get.mockImplementation((u: string) => Promise.resolve({
    data: u.includes('deliveries')
      ? [{ id: 3, event: 'article.created', statusCode: 200,
...
it('creates a webhook with chosen events', async () => {
  ...
  await waitFor(() => expect(m.post).toHaveBeenCalledWith('/api/webhooks',
    { url: 'https://a.io/x', events: ['comment.created'],
      active: true, tenant_id: 1 }))
})
...
it('edits without resending an empty secret', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: 'Save' }))
  await waitFor(() => expect(m.put).toHaveBeenCalledWith('/api/webhooks/7',
    { url: 'https://x.io/h', events: ['article.created', 'user.registered'],
      active: true }))
})
