...
jest.mock('@/components/admin/layout/AdminShell',
...
it('explains the requirement with a sign-in link', () => {
  render(<AdminGate><p>secret</p></AdminGate>)
  expect(screen.getByText('Administrator access required'))
    .toBeInTheDocument()
  expect(screen.getByTestId('admin-forbidden-signin')).toHaveAttribute(
    'href', '/auth/login?tenant=my%20site')
...
it('the admin layout is gated', () => {
  const { unmount } = render(<TenantAdminLayout><p>kid</p></TenantAdminLayout>)
