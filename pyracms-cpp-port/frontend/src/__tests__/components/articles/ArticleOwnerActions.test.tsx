...
beforeEach(() => Object.values(m).forEach(
  (f) => f.mockReset().mockResolvedValue({})))

const base: Article = {
  title: 'T', content: '', author: 'a', createdDate: '', renderer: 'html',
  views: 0, likes: 0, dislikes: 0, tags: [], revisionCount: 1,
  status: 'draft', isPrivate: false,
}
const changed = jest.fn()
const deleted = jest.fn()
const mount = (a: Partial<Article> = {}, tenantId: number | null = 3) =>
  render(<ArticleOwnerActions article={{ ...base, ...a }} name="n"
    tenantId={tenantId} onChanged={changed} onDeleted={deleted} />)
const click = (id: string) => fireEvent.click(screen.getByTestId(id))

describe('ArticleOwnerActions', () => {
  beforeEach(() => { changed.mockReset(); deleted.mockReset() })
  ...
  it('unpublishes a published article and toggles privacy', async () => {
    ...
    await waitFor(() => expect(m.post).toHaveBeenCalledWith(
      '/api/articles/n/unpublish', { tenant_id: 3 }))
  ...
  it('shows API errors and skips work without a tenant', async () => {
    ...
    const { unmount } = mount()
    click('article-publish-btn')
    expect(await screen.findByTestId('article-admin-error'))
