...
it('ArticleContent switches on renderer', () => {
  ...
  expect(screen.getByTestId('article-content').innerHTML)
...
it('ContentPreview shows content or placeholder', () => {
  const { rerender } = render(<ContentPreview content="<i>a</i>"
    renderer="HTML" />)
...
it('renderBBCode converts every tag', () => {
  ...
  for (const t of ['<strong>', '<em>', '<u>', 'href="http://a"', '<img',
    '<pre', '<blockquote', '<li>one</li>', 'color', 'font-size', '<br']) {
...
it('revision dialogs render and close', () => {
  ...
  const { rerender } = render(<RevisionViewDialog open onClose={jest.fn()}
    revision={rev} sanitizedContent="<b>c</b>" />)
  expect(screen.getByTestId('revision-content').innerHTML).toContain('<b>')
  rerender(<RevisionViewDialog open onClose={jest.fn()} revision={null}
    sanitizedContent="" />)
  render(<RevertConfirmDialog revisionNumber={4} onClose={jest.fn()}
    onConfirm={jest.fn()} />)
