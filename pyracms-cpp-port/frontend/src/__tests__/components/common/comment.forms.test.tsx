...
describe('EditForm and DeleteCommentDialog', () => {
  it('EditForm edits, saves and cancels', () => {
    const p = { setEditText: jest.fn(), onSave: jest.fn(), onCancel: jest.fn() }
    render(<EditForm editText="t" submitting={false} {...p} />)
    fireEvent.change(
      screen.getByTestId('comment-edit-input').querySelector('textarea')!,
      { target: { value: 'z' } },
    )
...
describe('CommentForm', () => {
  ...
  it('posts a comment then clears', async () => {
    ...
    const done = jest.fn()
    render(
      <CommentForm contentType="article" contentId={2} onSubmitted={done} />,
    )
    ...
    expect(post).toHaveBeenCalledWith('/api/comments/article/2', { body: 'yo' })
  })
