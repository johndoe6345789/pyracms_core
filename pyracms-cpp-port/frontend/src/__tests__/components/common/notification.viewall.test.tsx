...
beforeEach(() => {
  ...
  m.get!.mockImplementation((u: string) => Promise.resolve(
    u.includes('unread') ? { data: { count: 1 } } : { data: [{
      id: 5, type: 'reply', title: 'hey', message: 'm', link: '',
      isRead: false, createdAt: '2026-01-01T00:00:00Z' }] }))
})
...
it('links to the full page and reads the camelCase list', async () => {
  ...
  expect(screen.getByTestId('notifications-view-all'))
    .toHaveAttribute('href', '/site/s/notifications')
})
...
it('maps arrays, wrapped lists and junk', () => {
  expect(mapNotificationList({ notifications: [{ id: 1, is_read: 1 }] })[0])
