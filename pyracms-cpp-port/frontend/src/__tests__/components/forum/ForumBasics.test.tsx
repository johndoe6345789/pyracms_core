...
import {
  ForumEmpty, ForumError, ForumLoading,
...
it('renders a category with forum links', () => {
  render(<CategoryAccordion slug="s" category={{
    id: '1', name: 'General', forums: [
      { id: '2', name: 'Chat', description: 'd', threads: 3, posts: 4 },
      { id: '3', name: 'Dev', description: '', threads: 0, posts: 0 },
    ],
  }} />)
  expect(screen.getByText('General')).toBeInTheDocument()
  expect(screen.getByTestId('forum-link-2'))
...
it('renders breadcrumbs with and without links', () => {
  render(<ForumBreadcrumbs crumbs={[
    { label: 'Forum', href: '/f' }, { label: 'Now' },
  ]} />)
  expect(screen.getByText('Forum').closest('a'))
    .toHaveAttribute('href', '/f')
