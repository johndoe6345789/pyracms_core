...
const items = [
  { id: 1, title: 'Hello', snippet: 'a <b>needle</b> here', author: 'Ann',
    forumName: 'Tech', createdAt: '2024-03-04T00:00:00Z' },
...
describe('ForumSearchBar', () => {
  ...
  it('shows an empty message and toggles filters', async () => {
    ...
    fireEvent.change(screen.getByTestId('search-filter-author')
      .querySelector('input')!, { target: { value: 'a' } })
    fireEvent.change(screen.getByTestId('search-filter-date-from')
      .querySelector('input')!, { target: { value: '2024-01-01' } })
    fireEvent.change(screen.getByTestId('search-filter-date-to')
      .querySelector('input')!, { target: { value: '2024-02-01' } })
