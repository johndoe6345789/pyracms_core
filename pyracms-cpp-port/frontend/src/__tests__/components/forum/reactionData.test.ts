...
describe('toggleReaction', () => {
  it('adds a new reaction', () => {
    expect(toggleReaction([], 'x', 'lx')).toEqual([
      { emoji: 'x', label: 'lx', count: 1, reacted: true },
    ])
  })
  it('increments an unreacted one', () => {
    expect(
      toggleReaction([r('a', 1, false), r('b', 1, false)], 'a', 'a'),
    ).toEqual([r('a', 2, true), r('b', 1, false)])
  })
  it('decrements a reacted one and removes at zero', () => {
    expect(
      toggleReaction([r('a', 3, true), r('b', 1, false)], 'a', 'a'),
