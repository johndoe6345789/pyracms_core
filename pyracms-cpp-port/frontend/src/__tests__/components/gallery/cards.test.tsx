...
const album = {
  id: 3, name: 'Trip', coverImage: '/c.png', pictureCount: 7,
} as never

describe('gallery cards', () => {
  it('AlbumCard links to the album', () => {
    render(<AlbumCard album={album} slug="s" />)
    expect(screen.getByTestId('album-link-3'))
      .toHaveAttribute('href', '/site/s/gallery/3')
    expect(screen.getByText('7 pictures')).toBeInTheDocument()
    expect(screen.getByAltText('Cover image for album Trip'))
      .toBeInTheDocument()
  })

  it('AlbumGrid renders one card per album', () => {
    render(<AlbumGrid slug="s" albums={[album, { ...(album as object),
      id: 4 } as never]} />)
  ...
  it('PictureGrid renders picture links', () => {
    ...
    expect(screen.getByTestId('picture-item-9'))
      .toHaveAttribute('href', '/site/s/gallery/picture/9')
