...
describe('Get the launcher entries', () => {
  it('top bar tool defaults to /download, site passes its own', () => {
    ...
    expect(screen.getByTestId('get-launcher'))
      .toHaveAttribute('href', '/download')
    unmount()
    render(<TopBarTools downloadHref="/site/d/download" />)
    expect(screen.getByLabelText('Get the launcher'))
      .toHaveAttribute('href', '/site/d/download')
  })
  ...
  it('portal and site drawers list it, top-bar links do not', () => {
    ...
    expect(t.find((e) => e.key === 'download')?.href)
      .toBe('/site/d/download')
    render(<AppDrawer open onClose={() => {}} title="T" subtitle="S"
      sections={tenantSections('d', false)} />)
    expect(screen.getByTestId('drawer-download'))
      .toHaveTextContent('Get the launcher')
  })

  it('games header shows the banner only with a download href', () => {
    const base = { mobile: false, view: 'browse', onView: jest.fn(),
      onOpenDrawer: jest.fn() } as const
