...
describe('FollowerList', () => {
  ...
  it('shows empty state', async () => {
    ...
    expect(await screen.findByTestId('follower-list-empty')).toHaveTextContent(
      'No followers yet',
    )
  })
  ...
  it('loads more pages', async () => {
    get
      .mockResolvedValueOnce({ data: { items: [u(1)], total: 2 } })
  ...
  it('handles a response without items', async () => {
    ...
    render(<FollowerList userId={1} type="followers" />)
    expect(await screen.findByTestId('follower-list-empty')).toBeInTheDocument()
  })
})
