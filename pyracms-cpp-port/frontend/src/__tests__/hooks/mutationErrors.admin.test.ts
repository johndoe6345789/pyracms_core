...
const set = jest.fn()
const item = { id: 1, name: 'a', route: '/a', position: 0,
  permissions: 'public' }
const group = { id: 2, name: 'g', items: [item] }
beforeEach(() => { jest.resetAllMocks(); set.mockReset() })
...
it('menu item edit surfaces save and delete failures', async () => {
  ...
  await waitFor(() => expect(result.current.editError)
...
it('menu add item surfaces a failure', async () => {
  ...
  act(() => { result.current.setNewName('n'); result.current.setNewRoute('/') })
...
it('menu group create surfaces a failure', async () => {
  ...
  const { result } = renderHook(() =>
    useMenuGroupCreate(1, [], set, jest.fn()))
...
it('setting add surfaces a failure', async () => {
  ...
  act(() => { result.current.setNewKey('k'); result.current.setNewValue('v') })
...
it('file upload surfaces a failure and clears on retry', async () => {
  ...
  await waitFor(() => expect(result.current.uploadError)
    .toBe('boom'))
  act(() => result.current.uploadFiles([f] as unknown as FileList))
