...
beforeEach(() => {
  ...
  mock.get.mockResolvedValue({ data: {
    pinned: true, locked: false, posts: [{ id: 1 }],
  } })
...
it('votes, edits and deletes posts', async () => {
  ...
  await act(async () => { await result.current.handleVotePost('1', true) })
  expect(mock.post).toHaveBeenCalledWith('/api/forum/posts/1/vote', {
    isLike: true,
  })
  await act(async () => { await result.current.handleEditPost('1', 'n') })
  expect(mock.put).toHaveBeenCalledWith('/api/forum/posts/1', {
    content: 'n',
  })
  await act(async () => { await result.current.handleDeletePost('1') })
...
it('reports a vote failure', async () => {
  ...
  await act(async () => { await result.current.handleVotePost('1', false) })
  expect(result.current.replyError).toMatch(/vote/)
})

it('toggles pin and lock', async () => {
  const { result } = await setup()
  await act(async () => { await result.current.handleTogglePin() })
  expect(mock.put).toHaveBeenCalledWith('/api/forum/threads/10/flags', {
    pinned: false, locked: false,
  })
  await act(async () => { await result.current.handleToggleLock() })
...
it('deletes the thread', async () => {
  const { result } = await setup()
  await act(async () => { await result.current.handleDeleteThread() })
