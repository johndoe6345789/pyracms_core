...
describe('forumAdminRequest names the tenant on every action', () => {
  ...
  it('renames a category', async () => {
    await forumAdminRequest(
      { kind: 'category', mode: 'edit', id: '5' },
      3,
      'B',
      '',
    )
    expect(m.put).toHaveBeenCalledWith('/api/forum/categories/5', {
      name: 'B',
      tenantId: 3,
    })
  })
  it('deletes a category', async () => {
    await forumAdminRequest(
      { kind: 'category', mode: 'delete', id: '5' },
      3,
      '',
      '',
    )
    expect(m.delete).toHaveBeenCalledWith('/api/forum/categories/5?tenant_id=3')
  })
  it('creates a forum', async () => {
    await forumAdminRequest(
      { kind: 'forum', mode: 'create', parentId: '2' },
      3,
      'F',
      'd',
    )
  ...
  it('edits a forum', async () => {
    await forumAdminRequest(
      { kind: 'forum', mode: 'edit', id: '9' },
      3,
      'G',
      'e',
    )
    expect(m.put).toHaveBeenCalledWith('/api/forum/forums/9', {
      name: 'G',
      description: 'e',
      tenantId: 3,
    })
