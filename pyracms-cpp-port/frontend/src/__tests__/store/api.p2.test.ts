...
describe('rtk-query api failures and exports', () => {
  it('provides list tags even when the request fails', async () => {
    global.fetch = jest.fn(async () => new Response('{}', { status: 500,
      headers: { 'content-type': 'application/json' } })) as never
    const { store } = makeStore()
    const users = await store.dispatch(
      api.endpoints.getUsers.initiate() as never) as { isError: boolean }
    const tenants = await store.dispatch(
      api.endpoints.getTenants.initiate() as never) as { isError: boolean }
