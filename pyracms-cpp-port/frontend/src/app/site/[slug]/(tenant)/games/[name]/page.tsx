...
import { PLACEHOLDER_GAME_DETAIL } from '@/hooks/data/gamePlaceholders'

export default function GameDetailPage() {
  const params = useParams()
  const slug = params.slug as string
  const name = params.name as string
  const { detail, tabIndex, setTabIndex } = useGameDepDetail(PLACEHOLDER_GAME_DETAIL)

  return (
    <Container maxWidth="lg" sx={{ py: 6 }}>
      <Breadcrumbs separator={<NavigateNextOutlined fontSize="small" />} sx={{ mb: 3 }}>
        <Link href={`/site/${slug}/games`} style={{ color: 'inherit', textDecoration: 'none' }}>
          Games
        </Link>
      ...
      <GameDepDetail
        detail={detail}
        editHref={`/site/${slug}/games/${name}/edit`}
        tabIndex={tabIndex}
