...
async function siteEntries(
  origin: string, t: { id: number; slug: string },
): Promise<Entry[]> {
  const base = `${origin}/site/${encodeURIComponent(t.slug)}`
  const rows = await getJson<Row[]>(
    `/api/articles?tenant_id=${t.id}&limit=100`) ?? []
  return [{ url: base, changeFrequency: 'daily', priority: 0.9 },
    ...rows.map((a): Entry => ({
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const tenants = await getJson<{ id: number; slug: string }[]>(
    '/api/tenants') ?? []
  const groups = await Promise.all(
    tenants.map((t) => siteEntries(origin, t)))
