...
function countPosts(cats: Obj[]): number {
  return cats.reduce(
    (sum: number, c: Obj) =>
      sum +
      ((c.forums as Obj[]) || []).reduce(
        (s: number, f: Obj) => s + (Number(f.totalPosts) || 0),
        0,
      ),
    0,
  )
...
function countPics(albums: Obj[]): number {
  return albums.reduce(
    (s: number, a: Obj) => s + (Number(a.pictureCount) || 0),
    0,
  )
}

export function fetchFallback(tid: number): Promise<TrafficEntry[]> {
  ...
  return Promise.all([
    api
      ...
      .catch(() => 0),
    api
      .get(`/api/forum/categories${q}`)
      .then((r) => countPosts(r.data || []))
      .catch(() => 0),
    api
      .get(`/api/snippets${q}`)
      .then((r) => (r.data?.items || r.data || []).length)
      .catch(() => 0),
    api
