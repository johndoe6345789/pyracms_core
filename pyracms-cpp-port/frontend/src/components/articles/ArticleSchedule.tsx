...
export default function ArticleSchedule(p: Props) {
  ...
  return (
    <Box sx={{
      display: 'flex', gap: 1, alignItems: 'center', flexWrap: 'wrap', mt: 1,
      ...
      <TextField size="small" type="datetime-local" value={at}
        onChange={(e) => setAt(e.target.value)}
        slotProps={{ inputLabel: { shrink: true } }}
        label="Publish at" data-testid="article-schedule-input" />
      <Button size="small" disabled={p.busy || !at}
        onClick={() => p.onSchedule(at)} data-testid="article-schedule-btn">
        Schedule
      </Button>
      {scheduled && (
        <Button size="small" disabled={p.busy} onClick={p.onClear}
