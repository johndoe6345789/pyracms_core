...
export function PostCard({
  post, onVote, onEdit, onDelete, onQuote, canVote = true, tenantId = null,
}: PostCardProps) {
  const s = usePostCardState({ id: post.id, content: post.content,
    onEdit, onDelete })
  const stats = useUserStats(post.authorId, tenantId)
  return (
    <Paper variant="outlined" sx={{
      p: 3, borderColor: 'divider', display: 'flex', gap: 2,
      flexDirection: { xs: 'column', md: 'row' },
    }} data-testid={`post-card-${post.id}`}>
      {stats && <PostAuthorInfo username={post.author} stats={stats} />}
      <Box sx={{ flex: 1, minWidth: 0 }}>
        <PostCardHeader
          author={post.author} date={post.date} showAuthor={!stats}
          isOwner={post.isOwner} editing={s.editing}
          onSave={s.save} onCancelEdit={s.cancel}
          onStartEdit={() => s.setEditing(true)}
          onDelete={() => s.setConfirmDel(true)} />
        <ErrorAlert error={s.error} testId="post-error" />
        {s.editing ? (
          <MentionTextField fullWidth multiline minRows={3}
            value={s.editContent} onValue={s.setEditContent}
        ...
        <Box sx={{
          ...
          <PostReactions postId={post.id} disabled={!canVote}
      ...
      <PostDeleteDialog open={s.confirmDel}
        onClose={() => s.setConfirmDel(false)} onConfirm={s.remove} />
    </Paper>
  )
}
