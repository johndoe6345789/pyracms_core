...
export default function GameDepDetail({
  ...
  return (
    <>
      ...
      >
        <Typography variant="h3" component="h1">{detail.displayName}</Typography>
        <Button variant="outlined" startIcon={<EditOutlined />} component={Link} href={editHref}>
          Edit
        </Button>
      </Box>
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, mb: 2, color: 'text.secondary' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
          ...
          <Typography variant="body2">{detail.views.toLocaleString()} views</Typography>
        </Box>
      </Box>
      ...
      <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
        <Tabs value={tabIndex} onChange={(_, v) => onTabChange(v)} variant="scrollable" scrollButtons="auto">
          <Tab label="Revisions" />
          <Tab label="Binaries" />
