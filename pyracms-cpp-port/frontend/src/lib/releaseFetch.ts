...
interface Entry { at: number; release: LauncherRelease | null }
let memory: Entry | null = null

/** Test hook: forget the in-memory copy. */
export function resetReleaseCache() { memory = null }
...
function writeSession(e: Entry) {
  try { window.sessionStorage.setItem(KEY, JSON.stringify(e)) } catch { /* */ }
}

const fresh = (e: Entry | null): e is Entry =>
...
export async function fetchLatestRelease(): Promise<LauncherRelease | null> {
  ...
  if (fresh(saved)) { memory = saved; return saved.release }
  try {
